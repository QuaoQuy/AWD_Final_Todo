<div>
    ...
    <div class="flex gap-1">
        @forelse($categories as $category)
            <div class="flex my-4 text-white py-1 px-4 text-xs font-bold uppercase rounded-sm items-center justify-between"
                ...
                <div>
                    @if ($editCategory == $category->id)
                        <button class="opacity-50"
                            wire:click='updateCategoryButton({{ $category->id }})'>Update</button>
                    ...
                    @else
                        ...
                        <button class="opacity-50"
                            wire:click='deleteCategoryButton({{ $category->id }})'>Delete</button>
                    @endif
                </div>
        ...
        @empty
            <p class="text-gray-500">No cate created!</p>
        @endforelse
    </div>
